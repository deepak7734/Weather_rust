...
fn get_current_weather() {
    ...
    let weather_data = get_weather_data(&url).expect("Failed to get weather data");

    println!("Weather in {}:", weather_data["name"]);
    println!("  Temperature: {:.2}°C", weather_data["main"]["temp"]); // Format temperature with 2 decimal places
    println!("  Humidity: {}%", weather_data["main"]["humidity"]);
    println!("  Wind Speed: {:.2} m/s", weather_data["wind"]["speed"]); // Format wind speed with 2 decimal places
...
fn get_forecast() {
    ...
    let forecast_data = get_weather_data(&url).expect("Failed to get weather data");

    println!("Forecast:");
    for forecast in forecast_data["list"].as_array().expect("Invalid forecast data") {
        println!("  Date: {}", forecast["dt_txt"]);
        println!("    Temperature: {:.2}°C", forecast["main"]["temp"]); // Format temperature with 2 decimal places
        println!("    Humidity: {}%", forecast["main"]["humidity"]);
        println!("    Wind Speed: {:.2} m/s", forecast["wind"]["speed"]); // Format wind speed with 2 decimal places
        println!("    Description: {}", forecast["weather"][0]["description"]);
        println!("");
    }
}

fn get_weather_data(url: &str) -> Result<Value, reqwest::Error> {
